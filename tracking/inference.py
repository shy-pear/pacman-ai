...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # variables are given constants
    variables = [PAC, GHOST0, GHOST1, OBS0, OBS1]

    # edges are connections between variables represented as tuples
    edges = [(PAC, OBS0), (PAC, OBS1), (GHOST0, OBS0), (GHOST1, OBS1)]
    
    #position domains are all (x, y) coordinates for each agent
    positionDomain = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)] 
    variableDomainsDict[PAC] = positionDomain
    variableDomainsDict[GHOST0] = positionDomain
    variableDomainsDict[GHOST1] = positionDomain

    #observation domains are manhattan distances with max noise included
    observationDomain = range(X_RANGE + Y_RANGE + MAX_NOISE - 1)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # get factors conditioned on the evidenceDict
        factorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # for each variable in eliminationOrder
        for var in eliminationOrder:
            # join factors with variable
            factorsList, joinedFactor = joinFactorsByVariable(factorsList, var)

            # if joinedFactor has more than 1 unconditioned variable, eliminate var from factor
            # and append to factorsList
            joinedFactorLength = len(joinedFactor.unconditionedVariables())
            if joinedFactorLength > 1:
                factorsList.append(eliminate(joinedFactor, var))
        
        # get joint distribution of factorsList
        joinedFactors = joinFactors(factorsList)

        # return normalized version of joint factors
